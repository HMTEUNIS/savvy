...
function VeiwExisting({ art, setThread }) {
  ...
  const found = discussions.filter((dis) => {
    if (art.title === "") {
      ...
      dis.article_title.toLowerCase().includes(art.title.toLowerCase())
    ) {
      return dis;
    }
  });
  ...
  return (
    <div className="discuss">
      <header className="discussHead">
        Join a conversation. (or just feel free to lurk)
      </header>
      ...
      <br />
      {found.map((thread) => (
        <ThreadCard setThread={setThread} whole={thread} />
      ))}
    </div>
  );
}
