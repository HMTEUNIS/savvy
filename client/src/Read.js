...
function Read({ setter }) {
  ...
  const handleSearch = (event) => {
    ...
    if (searchTerm.length > 1) {
      fetch(
        `https://core.ac.uk:443/api-v2/articles/search/${searchTerm}?page=1&pageSize=100&metadata=true&fulltext=false&citations=false&similar=false&duplicate=false&urls=true&faithfulMetadata=false&apiKey=BKLkomgqtA2N8cVRws9nbYjS1pvFy07d%20`
      )
        .then((res) => res.json())
        .then((data) => {
          setData(data);
  ...
  return (
    <div>
      ...
      {data.length < 1 ? (
        <div>
          <p>From the CORE website:</p>
          <p>
            CORE's mission is to aggregate all open access research worldwide
            and deliver unrestricted access for all. In doing so, we:
          </p>
          <ul>
            <li>
            ...
            <li>
              enable others to develop new tools and use cases on top of the
              CORE platform,
            </li>
            <li>
              support the network of open access repositories and journals with
        ...
        <div>
          <p>Total Results: {data.totalHits} </p>
          <p>
            We only display the first hundred. You gotta dig in from there. If
            you find something you can still start a thread about it! ||{" "}
          </p>
      ...
      <div className="disArt">
        {data.length < 1 ? (
          ...
          data.data.map((article) => (
            <ArticleCard setter={setter} article={article} />
          ))
        )}
      </div>
    </div>
