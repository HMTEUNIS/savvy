...
function Thread({ thread, setRe, re, currentUser, poster, setPoster }) {
  ...
  function handleSubmit(e) {
    ...
    fetch("/posts", {
      ...
      .then((window.location.href = "/discuss"));
  }
  ...
  return (
    <>
      <div>
        ...
        <a onClick={linkClick} className="thread_art">
          {display.article_title} by: {thread.author} || CORE ID{" "}
          {thread.core_id}{" "}
        </a>
      </div>
      ...
      {thread.posts.map((comment) => (
        <div className="eachPost">
          ...
          <div>
            ...
            <header className="postby">{comment.user.user_name} </header>
          </div>
          <div className="commentDisplay"> {comment.comment} </div>{" "}
          {currentUser ? (
      ...
      <br />
      {currentUser ? (
        <img className="thread_user_img" src={currentUser.img_url} />
      ) : null}
        ...
        <>
          ...
          <div>
            <form onSubmit={handleSubmit}>
              <textarea
                className="postForm"
                value={Trec ? talkText : console.log("stuff")}
