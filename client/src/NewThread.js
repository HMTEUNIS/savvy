...
function NewThread({ art, currentUser }) {
  ...
  return (
    <div>
      {currentUser ? (
        <>
          <span className="ntt">{art.title}</span>
          <p className="nta"> by: {art.authors} </p>
          <p className="ntid"> Article CORE id: {art.id}</p>
          <form onSubmit={postSubmit}>
            ...
            <textarea
              className="postForm"
              value={NTrec ? talkText : console.log(false)}
              placeholder="whats on your mind?"
              onChange={(e) => setBody(e.target.value)}
            ></textarea>
            <input type="submit" />
          </form>
          <div onClick={NTrecord}>
            {" "}
            <Talk setTT={setTT} />{" "}
          </div>{" "}
        </>
